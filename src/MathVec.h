...
template <class T>
class MathVec2 : public MathVec<T, 2U> {
public:
	MathVec2(T a, T b);
	MathVec2(const MathVec2&);
	MathVec2(const MathVec<T, 2>&);
	MathVec2<T>& operator=(const MathVec<T, 2>& v2);
};


template <class T>
class MathVec3 : public MathVec<T, 3U> {
public:
	MathVec3(T a, T b, T c);
	MathVec3(const MathVec3&);
	MathVec3(const MathVec<T, 3>&);
	MathVec3<T>& operator=(const MathVec<T, 3>& v2);
};
...
template<class T>
inline MathVec3<T>::MathVec3(const MathVec<T, 3>& source) : MathVec<T, 3>(source.get(0)) {
	this->data[1] = source.get(1);
	this->data[2] = source.get(2);
}

template<class T>
inline MathVec3<T>& MathVec3<T>::operator=(const MathVec<T, 3>& source) {
